...
def record_tip_activity(tip, github_handle, event_name):
    ...
    try:
        ...
        logging.error(f"error in record_tip_activity: {e} - {event_name} - {tip} - {user}")
...
def send_tip_2(request):
    ...
    if request.body:
        ...
        if not to_emails:
            ...
            response['message'] = _(
                'Uh oh! No email addresses for this user were found via Github API.  Youll have to let the tipee know manually about their tip.')
